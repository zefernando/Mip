...
int MIP_Session(_Run * Run)
{
	...
	/* sprintf(Log_Msg,"Completion Code for run is %d",rc); */
	sprintf(Log_Msg,"Completion Code");
	Log_This(Log_Msg, mpLOG_NORMAL); 
...
int main(int argc, char *argv[])
{
	...
	/* Abre o terminal */
	Run.fd_in = open("/dev/tty",O_RDWR|O_NDELAY);
	...
	if (!Run.bOutput)
	{
		...
		if ( param == NULL )
		{
			...
			)
			{
				...
				Run.Output_File = malloc(8);
				memcpy(Run.Output_File,"mipAdvises.txt",8);
	...
	if ( param != NULL )
	{
		...
		if (!(rc = stat(param,&stats)))
		{
			...
			Log_This(Log_Msg, mpLOG_NORMAL);
			Run.bTest_Mode = 1;
			Run.fd_Temp = open(param,O_RDONLY);
